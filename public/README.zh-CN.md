...
可选的 Photo Sphere Viewer 插件包括罗盘、陀螺仪、图片地图、地理地图（默认 OpenStreetMap）、立体视图和可见范围。插件代码仅在启用时加载；立体视图会自动加载其依赖的陀螺仪。地图图片和地理地图瓦片模板支持绝对或相对 URL（瓦片模板保留 `{z}`、`{x}`、`{y}`）。

其他功能包括配置初始水平角、俯仰角、缩放、视野、移动速度和导航控件；配置导航栏标题与描述；配置空闲自动旋转；配置视频自动播放和静音；按挂件实例存储配置；自动跟随思源的明暗外观。拖动、滚轮、Ctrl+滚轮和双指交互开关位于 Photo Sphere Viewer 原生“设置”菜单中，并会立即保存。
